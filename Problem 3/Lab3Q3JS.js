function button_action()
{
	...
	function add_note_function()
	{
		...
		var add_child_note = document.createElement("input");
		add_child_note.type = "button";
		add_child_note.className = "button";
		add_child_note.value = "Add another Note";


		node.appendChild(add_child_note);
		
		rxjs.fromEvent(add_child_note, 'click')
			.subscribe(() => adding_child_note_to_parent()
		);
	}
	document.getElementById("DisplayNote").appendChild(node);
	
	function adding_child_note_to_parent()
	{
		node.innerHTML += title_content + "<br><br><br>" + note_content;
		node.appendChild(document.createTextNode('\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0' ));
		node.appendChild(input);
